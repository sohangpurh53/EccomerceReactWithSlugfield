...
const UpdateProductForm = () => {
        ...
        const fetchData = async ()=>{
            try {
                const [categoryData, sellerData, product] = await Promise.all([
                    ...
                    axiosInstance.get(`update/product/${slug}/`).then(response=> response.data),
                ]);
    ...
    const CreateProduct = async (e)=>{
        ...
        console.log(data)
        try {
            await axiosInstance.put(`update/product/${slug}/`, data).then(response=> { if(response.data){
    ...
    <div className="product-form-container">
       <Notfication message={notification}/>
        <form >
           ...
           <button onClick={CreateProduct}>Update Product</button>
        </form>
    </div>
    </>
  )
